...
def index_all_from_env(
    *,
    ...
    mapping = {
        "rta_act": (os.getenv("INDEX_RTA_ACT", ""), os.getenv("INDEX_RTA_ACT_DRIVE_URL", "")),
        "sppa_act": (os.getenv("INDEX_SPPA_ACT", ""), os.getenv("INDEX_SPPA_ACT_DRIVE_URL", "")),
        "ltb_rules": (os.getenv("INDEX_LTB_RULES", ""), os.getenv("INDEX_LTB_RULES_DRIVE_URL", "")),
        ...
        "ltb_forms": (os.getenv("INDEX_LTB_FORMS", ""), os.getenv("INDEX_LTB_FORMS_DRIVE_URL", "")),
        "form_instructions": (
    ...
    for key, (index_name, drive_url) in mapping.items():
        if allowed is not None and key not in allowed:
            continue
        index_name = (index_name or "").strip()
        drive_url = (drive_url or "").strip()
        if not index_name or not drive_url:
            results[key] = {"skipped": True, "reason": "missing index name or drive url"}
            continue
